...
//------------------employee functions start ---------------//
    ...
    public function employee_by_designation(Request $request)
    {
        if (!$this->check_user()) {
            ...
            $offices = Officeinfo::where('designation_id', $designation_id)->get();

            $html = "<option value=''>Select</option>";
            foreach ($offices as $office){
                $html .= '<option value="'.$office->employee->id.'">'.$office->employee->first_name.' '.$office->employee->last_name.'</option>';
            }
            echo $html;
        }
